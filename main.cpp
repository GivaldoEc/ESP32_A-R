...
typedef enum {INIT, AV, AR, MENU, RUN, SAVE}  state;    //main states
typedef enum {START_, WAIT_30, WAIT_C1, WAIT_C2, WAIT_100, END_RUN} run_state;  //states from the run mode
typedef enum {RESET, SIM, NAO} sv_state; //states from the save mode
...
void loop() {
    ...
    switch (ss) {
        ...
        case MENU:                  //Global Menu, where the user chooses which module go through
            ...
            if ( !digitalRead(B_SEL) )   { //Check if the button is pressed
                delay(10);
        ...
        case SAVE:              // Save on SD
            ...
            if ( !digitalRead(B_SEL) )   {
                delay(10);
                while(!digitalRead(B_SEL)) { //Check if the button is pressed
...
void GlobalMenu () {
    pos[0] = 17; pos[1] = 23;
    lcd.setCursor(0,0);
    if (controlmode == AV) { //Check if it is on AV mode
        lcd.print("   Modulo AV");
    } 
    else {
        lcd.print("   Modulo AR");
    ...
    if ( !digitalRead(B_CANC)) { //See if the cancel button is active
        delay(10);
        while(!digitalRead(B_CANC)) {
            ss = MENU;
            lcd.clear();
    ...
    if ( !digitalRead(B_SEL) )   { //See if the select button is active
        delay(10);
        while(!digitalRead(B_SEL)) {
